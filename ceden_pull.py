...
@exception_handler
def ceden_pull(datatype, link, eng, *args, cutoffyear = 1999, skip_download = False, **kwargs):
    ...
    if (not skip_download) or (not os.path.exists(newfolder)):
    ...
    create_tbl_sql = """
            DROP TABLE IF EXISTS ceden_{} CASCADE; 
            CREATE TABLE ceden_{} ({});
        """.format(
            datatype,
            datatype,
